...
export default function listUser() {
  ...
  const [isViewCustomerHasPetModal, setViewCustomerHasPetModal] =
    useState(false)
  ...
  function validarPassword(password: string, password2: string) {
    ...
    if (password.length >= 1 || password2.length >= 1) {
      ...
      if (!passwordValido.test(password)) {
        mensajePassword =
          'la contraseña tiene que tener letras, números, un minimo de caractes de 6 y un máximo de 16 '
        validador = false
      }
  ...
  function validarYear(year: string) {
    ...
    if (anoValido > 7 && anoValido <= 100) {
      ...
      validador = false
      mensajeYear =
        'La edad de nacimiento no puede ser mayor a 100 ni menor a 7'
    }
